...
const myArray2 = new Array(1, 2, 3) // тоже массив но записывается иначе
// console.log(array[0])  // так обращаемся к элементам массива
...
// array[2] = ['измененный элемент']
// array[6] = ['добавление в масив']  // тут пропустили очередность в массиве . свободные ячейки приняли значение undefined.  длинна пересчиталась по последнему (увеличилась)
// array.push(добавляемое значение)        добавление в конец массива.возвращает новую длину массива
// array.shift()     ИЗВЛЕКАЕМ начальный(нулевой) элемент массива.функция вернет значение этого элемента, а элемент исчезнет из массива
// array.pop()       ИЗВЛЕКАЕМ последний элемент массива.функция вернет значение этого элемента, а элемент исчезнет из массива
...
if (age3 != null) {
	alert('мой возраст ' + age3)
} else {
	alert('вы нажали отмену')
}

confirm('нажмите клавишу') ? alert('вы нажали ОК') : alert('вы нажали отмена')      //тринарный оператор

let age1 = prompt('введите возраст')
if (age1 <= 0) {                                              //тут добавили проверке отрицательноого числа
	alert('вашвозраст равен или меньше 0')
} else if (age1 < 18) {
...
//когда много ререключателей используй switch
let age2 = prompt('введите возраст')
switch (true) {
	case age2 < 18 && age2 > 0:                  // тут с помощью логического и отсекаем отрицательные числа  true && false
		alert('мой меньше  меньше 18')
		break;
	case age2 == 18:
		alert('мне 18')
		break;
	case age2 > 18:
		...
		break;
	default:
		alert('вы ввели текст  0 или отрицательное число')
}
...
for (let i = 0; i < 11; i = i + 2) {       // i++   вводим переменную i и присваиваем ей значение; пишем условие которое проверяем, если истино(true) выполняем код.
	console.log(i)                       // в данно примере выведем четные числа  i += 2
}

const arr1 = [5, 7, 9, 1, 0];                            // циклы в массивах
for (let item1 of arr1) {                                // в созданную переменную item1 передаются поочередно все элементы массива
	console.log(item1 * 2)                                // например каждый можем умножить на 2
}


["Бильбо", "Гэндальф", "Назгул"].forEach((item, index, array) => {        //Перебор: forEach Метод arr.forEach позволяет запускать функцию для каждого элемента массива.
	alert(`У ${item} индекс ${index} в ${array}`);
});


let arrayLike = {                                  // псевдомассив не работают методы массивов например pup()
	0: "Hello",
	1: "World",
	length: 2
...
let arr = Array.from(arrayLike);   //     метод Array.from, который принимает итерируемый объект или псевдомассив и делает из него «настоящий» Array. После этого мы уже можем использовать методы массивов
alert(arr.pop());                  //     Технически итерируемые объекты должны иметь метод Symbol.iterator.
...
for (let key1 in obj1) {                     // циклы в объектх. в созданную нами переменную key1 пооочередно передаются имана(ключи) обектов. не использовать в массивах
	...
	console.log(obj1[key1])                   // зная имена (ключи) обекта получаем возможность обращаться к еначениям и производить вычисления
